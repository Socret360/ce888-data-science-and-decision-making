...
def read_raw_measurement_file(filepath, columns=None):
    ...
    df = pd.read_csv(
        filepath,
        header=1,
        names=columns if columns is not None else list(range(len(info_df.iloc[0])))
    )

    df['timestamp'] = df.apply(lambda x: start_time + (x.name / sampling_rate), axis=1)
...
def get_experiment_start_end_time(participant):
    ...
    for measurement in ["ACC.csv", "TEMP.csv", "HR.csv", "BVP.csv", "EDA.csv"]:
        df, start_time, end_time, _ = read_raw_measurement_file(os.path.join(participant, measurement))
        t.append({
    ...
    # IBI.csv
    df, start_time, end_time = read_ibi_measurement_file(os.path.join(participant, "IBI.csv"))
    t.append({
        "measurement": "IBI.csv",
        "start_time": start_time,
        "end_time": end_time
...
def adjust_measurement_sampling_rate(measurement, start_time, end_time, sampling_rate):
    ...
    """
    num_seconds = int(end_time - start_time) + 1  # The number of seconds 0->1 = 2 seconds

    steps = np.arange(0, 1, step=1/sampling_rate)
    seconds = np.arange(start_time, end_time+1, step=1)

    timestamps = pd.DataFrame({"timestamp": np.tile(steps, (num_seconds)) + np.repeat(seconds, repeats=sampling_rate, axis=0)})

    return pd.merge(timestamps, measurement, on=['timestamp'], how='left')  # keep all timestamps, if row in measurement does not have corresponding timestamp, then drop
...
def read_participant_task_tags(filepath):
    ...
    """
    df = [float(i[0]) for i in pd.read_csv(filepath, header=None).values.tolist()]
...
def determine_timestamp_stress_status_of_participant(measurements, task_tags):
    ...
    def duration_stress_task(x):
        ...
        for (start_time, end_time) in task_tags[:3]:  # only select the first three tasks because they are the stress inducing task
...
def combine_participant_measurements(participant, sampling_rate=1):
    ...
    for measurement in ["ACC.csv", "TEMP.csv", "HR.csv", "BVP.csv", "EDA.csv"]:
        ...
        df = adjust_measurement_sampling_rate(df, start_time, end_time, sampling_rate)
        df = df.fillna(method='ffill')  # handle na values generated by upsampling
    ...
    combined = reduce(lambda left, right: pd.merge(left, right, on=['timestamp'], how='left'), measurements)  # keep rows without timestamp
...
def plot_experiment_measurement_as_timeseries(df, measurement):
    ...
    events = list(zip([('Stroop CW', 'r'), ('Trier Social Scale Test', 'g'), ('Hyperventilation Tests', 'b')], events))
    ...
    plt.axhline(y=avg_stress, label=f'Avg Stress ({round(avg_stress, 2)})', color='r')
    plt.axhline(y=avg_non_stress, label=f'Avg Non Stress ({round(avg_non_stress, 2)})', color='b')

    # draw shades over events
    for event in events:
        (name, color), (start_time, end_time) = event
        duration = int((end_time - start_time) // 60)
        plt.axvspan(start_time, end_time, alpha=0.1, lw=0, label=f"{name} ({duration} mins)", color=color)
